 ...
 * @return false 失败
 */
extern bool internal_plat_erase(const char *part_name, size_t offset, size_t len);

/**
 * @brief 向指定分区指定偏移处写入数据
 * @param part_name 分区名称
 * @param offset    分区内的字节偏移
 * @param buf       待写入数据缓冲区
 * @param len       待写入字节长度
 ...
 * @return false 失败
 */
extern bool internal_plat_write(const char *part_name, size_t offset, const uint8_t *buf, size_t len);

/**
 * @brief 从指定分区指定偏移处读取数据
 * @param part_name 分区名称
 * @param offset    分区内的字节偏移
 * @param buf       接收数据缓冲区
 * @param len       待读取字节长度
 ...
 * @return false 失败
 */
extern bool internal_plat_read(const char  *part_name, size_t offset, uint8_t *buf, size_t len);
