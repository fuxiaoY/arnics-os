...
{
    if(true == internal_plat_erase("rolldb_tsdb", address, SINGLE_BLOCK_SIZE))
    {
        return 0;
    }
    return -1;
...
static int tsdb_write_data(uint32_t address, void *data, uint32_t length)
{
    ...
    while (length > 0)
    {
        ...
        // 写入当前页
        if(true != internal_plat_write("rolldb_tsdb", address, data_ptr, bytes_to_write))
        {
            return -1; // 写入失败
        }
...
{
    if(true == internal_plat_read("rolldb_tsdb", address, (uint8_t*)data, length))
    {
        return 0;
    }
    return -1;
